...
export async function extractDataFromText(
  ...
  try {
    ...
    const dataExtractionPrompt = `You are an expert data extraction AI. Your task is to analyze user text and extract factual statements about the user. You must then format these facts into a JSON object that can be used to populate a knowledge graph.
...
2. \`new_facts\`: A list of fact objects to be created. Each fact must link a 'user_name' to a 'concept_name' and can include optional 'details'.
...
User Text: ${text}`;
    ...
    try {
      ...
      userId = user.id;
      console.log('Created/found user:', userId);
    } catch (error) {
